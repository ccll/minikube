...
func init() {
	...
	startCmd.Flags().String(imageRepository, "", "Alternative image repository to pull docker images from. This can be used when you have limited access to gcr.azk8s.cn. Set it to \"auto\" to let minikube decide one for you. For Chinese mainland users, you may use local gcr.azk8s.cn mirrors such as registry.cn-hangzhou.aliyuncs.com/google_containers")
